...
select count(*) - sum(customer_not_cancelled) searches from trips_details4;
...
SELECT * FROM PAYMENT;
select distinct tripid from trips
where fare = ( select max(fare) from trips)

select a.method from payment a
INNER JOIN (
select distinct tripid ,faremethod, fare from trips
where fare = (select max(fare) from trips)
) 
b on a.id = b.faremethod;
...
select sum(searches_got_quotes)*100.0/sum(searches_got_estimate) from trips_details4;
...
select sum(otp_entered)*100.0/sum(searches_got_quotes) from trips_details4;


-- booking cancellation rate
select sum(searches_got_quotes-otp_entered)*100.0/sum(searches_got_quotes) from trips_details4;


-- conversion rate
select sum(end_ride)*100.0/sum(searches) from trips_details4;
...
group by duration, loc_from ) as a
)as b 
where rnk = 1 ;
...
select * 
from (
select *, rank() over (order by can desc) as rnk 
from (
